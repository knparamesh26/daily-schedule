  ...
  return (
    <div className="p-xl max-w-5xl mx-auto w-full">
      <div className="flex items-start justify-between mb-xl">
        <div>
          <h2 className="text-page-title text-on-surface">Active Projects</h2>
        ...
        <div className="flex items-center gap-sm px-md py-xs border border-outline-variant rounded-lg bg-surface-container-lowest shadow-sm">
          <span className="material-symbols-outlined text-on-surface-variant text-icon-md">search</span>
          <input
      ...
      <div className="space-y-sm mb-xl">
        {filtered.map((project, i) => {
          ...
          return (
            <div
              key={project.id}
              className="bg-surface border border-outline-variant rounded-xl px-lg py-md flex items-center gap-lg shadow-sm hover:shadow-md transition-shadow"
            >
              <div
                className="w-12 h-12 rounded-xl flex items-center justify-center shrink-0"
                style={{ backgroundColor: project.color + '22' }}
              >
                <span className="material-symbols-outlined text-icon-xl" style={{ color: project.color }}>
              ...
              <div className="w-40 shrink-0">
                ...
                {project.description && (
                  <p className="text-label-sm uppercase tracking-wider mt-0.5 truncate text-on-surface-variant/60">
                    {project.description}
                  </p>
                )}
              </div>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="w-full bg-surface-container-high rounded-full overflow-hidden h-1.5">
                  <div
              ...
              <div className="flex items-center gap-xs shrink-0">
                {overdue > 0 && (
                  <span className="flex items-center gap-xs text-label-sm text-error bg-error/10 px-sm py-0.75 rounded-full font-medium">
                    <span className="material-symbols-outlined text-icon-xs">warning</span>
                    {overdue} overdue
                  </span>
                )}
                {inProgress > 0 && (
                  <span className="flex items-center gap-xs text-label-sm text-primary bg-primary/8 px-sm py-0.75 rounded-full">
                    <span className="material-symbols-outlined text-icon-xs">play_circle</span>
                    {inProgress} active
                  </span>
                )}
                {nearestDue && (
                  <span className="flex items-center gap-xs text-label-sm text-on-surface-variant bg-surface-container px-sm py-0.75 rounded-full">
                    <span className="material-symbols-outlined text-icon-xs">calendar_today</span>
                    {nearestDue}
                  </span>
                )}
              </div>
              ...
              <div className="flex items-center gap-lg shrink-0 ml-lg">
                ...
                >
                  <span className="material-symbols-outlined text-icon-lg">delete</span>
                </button>
              </div>
            </div>
          );
        ...
        >
          ...
          <div className="text-center">
            ...
            <p className="text-label-sm text-on-surface-variant mt-0.5">Define goals, milestones, and track progress.</p>
          </div>
        </button>
      </div>
  ...
  return (
    <div className="bg-surface border border-outline-variant rounded-xl p-lg">
      <div className="flex items-start justify-between mb-md">
        <span className="material-symbols-outlined text-on-surface-variant text-icon-xl">{icon}</span>
        <span className={`text-label-sm font-semibold px-sm py-0.5 rounded-full ${badgeStyle}`}>{badge}</span>
      </div>
